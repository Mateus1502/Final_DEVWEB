...
function validaRespostas(respostasDoUsuario) {
  ...
  for (let [key, value] of respostasDoUsuario) {
    ...
    console.log(quizz);
    const pergunta = quizz.perguntas.find((item) => item.name == key);
    if (pergunta) {
      const resposta = pergunta.respostas.find((item) => item.value == value);
      if (resposta.certo) {
        acertos++;
      }
    }
  ...
  window.location.href = `/resultado?quantidade_acertada=${acertos}&quantidade_perguntas=${quantidade_perguntas}`;
}
